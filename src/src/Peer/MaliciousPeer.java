 ...
 */
public class MaliciousPeer extends UnicastRemoteObject implements Peer
{
    ...
    public MaliciousPeer(String centralIp, int capacity, int ioSpeed) throws RemoteException {
        super();
    ...
    public static void main(String args[]) throws Exception {
        ...
        try {
            centralIp = args[0];
            capacity = calculateCapacity(); //returns max now: 10
            if(capacity < 1){
            ...
            System.out.println("The format is expected to be: <Central IP> <capacity: int [1, 10]> <io Speed :Int [1, 10]>"); // <registry port: int>
            return;
        }
    ...
    public <T> T executeTask(Task<T> task) throws RemoteException {
        ...
        try {
            ...
            if (result instanceof int[][]) {
                ...
                int[][] maliciousResult = MatrixGenerator.generateMatrix(length);
                return (T)maliciousResult;
            }
    ...
    public <T> byte[] executeAndEncryptTask(byte[] encryptedTask) throws RemoteException {
        try {
            ...
            if (result instanceof int[][]) {
                ...
                return aesCipher.doFinal(Serializer.serialize(maliciousResult));
            }
